...
配套：[完整张量流程图](tensor_flow.md)（整体 forward、Block 内部、KV cache，以及每步维度和含义）。

## 0. 为什么值得研究 KV cache？（报告第 2-3 页）

推理服务希望在延迟可接受的前提下，用有限硬件处理更多请求。报告用 Meta 的
2026 年资本支出预测说明基础设施规模，用 AWS/vLLM 和 LinkedIn/SGLang 说明
框架的实际采用。来源和适用范围见[背景资料](references.md#industry-background-for-slides-2-3)。
LinkedIn 的案例是只做 prefill 的排序服务，不是逐 token 生成聊天回答。

我们的切入点很具体：朴素生成每一步重算整个历史；KV cache 保存历史 K、V，
让模型只计算新位置，代价是占用缓存内存。历史 K、V 仍然需要参与 attention，
所以缓存不会消除所有计算，也不会消除显存带宽瓶颈。GPU 上的小 batch decode
常常受数据读取速度限制，长上下文和高并发还会增加缓存容量压力。

报告中的“50% fewer GPUs”来自 vLLM 团队 2023 年披露的 LMSYS 部署案例，
是整个服务系统的改进结果。它不是“打开普通 KV cache 就节省一半 GPU”，
也不是我们的小模型实验结果。工业框架已经有缓存；我们是在理解其基础机制。

回答“能省多少钱”时，可以说明更高吞吐量可能减少所需 GPU 小时，或推迟扩容。
实际金额还取决于负载、价格、延迟要求和硬件能否缩减。不要拿我们的 2.40 倍
CPU 加速直接推算大公司的费用。假设算例放在[英文讲稿末尾](speaker_notes.md)，
不占正式展示时间。
